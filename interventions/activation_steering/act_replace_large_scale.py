...
if __name__ == "__main__":
    ...
    ## final plotting
    def create_custom_plots(combined_data, baseline_dict, categories, window_size=10, 
                            model='chat', outdir="custom_plots"):
        """Create 2x3 grid of plots with specified formatting"""
        ...
        for row_idx, identifier in enumerate(identifiers):
            for col_idx, category in enumerate(categories):
                ...
                try:
                    df = combined_data[identifier][model][category][window_size]
                except KeyError:
                    ax.text(0.5, 0.5, "No Data", ha='center', va='center')
                    continue
                    
                # Filter for race injection
                race_data = df[df['Activation'] == 'Race']
                
                if race_data.empty:
                    ax.text(0.5, 0.5, "No Race Data", ha='center', va='center')
                    continue
                    
                # Plot main proportion line
                ax.plot(race_data['Window'], race_data['Proportion'], 
                        'k-o', markersize=5, linewidth=1.5, label='Race-injected')
                
                # Plot confidence intervals
                ax.plot(race_data['Window'], race_data['CI_lower'], 
                        color='grey', linestyle='-', linewidth=1)
                ax.plot(race_data['Window'], race_data['CI_upper'],
                        color='grey', linestyle='-', linewidth=1, label='95% CI')
                
                # Add baseline
                try:
                    if identifier == 'default':
                        baseline = baseline_dict[identifier][category][model]
                    else:
                        baseline = baseline_dict[identifier][category][model]['implicit']['mean']
                    ax.axhline(y=baseline, color='lightcoral', linestyle='-', 
                            linewidth=1.5, label='Baseline')
                except KeyError:
                    pass
                
                # -- Only add title for top row (row_idx = 0) --
                if row_idx == 0:
                    ax.set_title(f"{category}", pad=10)
                
                # -- Only add y-axis label for leftmost two columns --
                if col_idx in [0]:
                    ax.set_ylabel("Black Implicit Bias Level", labelpad=8)
                
                # -- Only add x-axis label for bottom row (row_idx = 1) --
                if row_idx == 0:
                    ax.set_xlabel("Window Index", labelpad=8)
                
                # Keep y-limits and grid
                ax.set_ylim(0.2, 1.03)
                ax.axhline(y=0.5, color='gray', linestyle='--', label = "Unbiased Level", linewidth=1)
                ax.grid(True, alpha=0.3)
                
                # Add legend to the very first subplot only
                if col_idx == 0:
                    ax.legend(loc='lower right', framealpha=0.9)

        # Add row labels (Default, Uppercase) on the far left, 
        # with a bit of extra negative xytext to space them out from y-axis labels.
        for row_idx, label in enumerate(['Default']):
            axes[0].annotate(
                label, xy=(0, 0.5), xytext=(-60, 0),  # move a bit further left
                xycoords='axes fraction', textcoords='offset points',
                ha='center', va='center', rotation=90, fontsize=12
            )

        plt.tight_layout(rect=[0.06, 0, 1, 0.95])
        plt.savefig(os.path.join(outdir, f'race_injection_comparison_window{window_size}.pdf'), 
